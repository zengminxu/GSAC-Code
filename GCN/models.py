...
class GCN(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, dropout):
        ...
        self.W1 = nn.Parameter(torch.empty(size=(in_dim, 2 * hid_dim)))
        nn.init.xavier_normal_(self.W1.data)
        self.W2 = nn.Parameter(torch.empty(size=(2 * hid_dim, hid_dim)))
    ...
    def forward(self, input, adj):
        ...
        h1 = torch.mm(adj, torch.mm(x, self.W1))
#        h1 = F.elu(h1)
        h1 = F.dropout(h1, self.dropout, training=self.training)
        h2 = torch.mm(adj, torch.mm(h1, self.W2))
#        h2 = F.elu(h2)
